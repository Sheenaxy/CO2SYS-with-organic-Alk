function [DATA,HEADERS,NICEHEADERS]=CO2SYSm(PAR1,PAR2,PAR1TYPE,PAR2TYPE,SAL,TEMPIN,TEMPOUT,PRESIN,PRESOUT,SI,PO4,pHSCALEIN,K1K2CONSTANTS,KSO4CONSTANTS,ORG,PKORG);%**************************************************************************
%
% This is a modified CO2SYS with (monoacid) organic acid total
% concentration and pK values as inputs. This is only a simple simulation
% without taking the influence of T, S, P, and ligand/complexation
% reactions into the consideration. This version is only a prototype,
% updated files can be found in https://github.com/Sheenaxy/Dyeperturbation
%Any question please contact xinyuli@Udel.edu
...
%        ...K1K2CONSTANTS,KSO4CONSTANTS,ORG,PKORG)
% 
%  **** SYNTAX EXAMPLES:
%  [Result]                     = CO2SYS(2400,2200,1,2,35,0,25,4200,0,15,1,1,4,1,4,8)
%  [Result,Headers]             = CO2SYS(2400,   8,1,3,35,0,25,4200,0,15,1,1,4,1,4,8)
%  [Result,Headers,Niceheaders] = CO2SYS( 500,   8,5,3,35,0,25,4200,0,15,1,1,4,1,4,8)
%  [A]                          = CO2SYS(2400,2000:10:2400,1,2,35,0,25,4200,0,15,1,1,4,1,4,8)
%  [A]                          = CO2SYS(2400,2200,1,2,0:1:35,0,25,4200,0,15,1,1,4,1,4,8)
%  [A]                          = CO2SYS(2400,2200,1,2,35,0,25,0:100:4200,0,15,1,1,4,1,4,8)
%  
...
%   DYE   (umol/kgSW) : scalar or vector of size n 
%   pHSCALEIN         : scalar or vector of size n (**)
%   K1K2     : scalar or vector of size n (***)
%   KSO4     : scalar or vector of size n (****)
%   OrgAlk   : scalar or vector of size n 
%   PK       :scalar or vector of size n 
%
...
%     '13 - ORGALKin         (umol/kgSW) '; %%New
%     '14 - Hfreein          (umol/kgSW) ';
%     '15 - RevelleFactorin  ()          ';
%     '16 - OmegaCain        ()          ';
%     '17 - OmegaArin        ()          ';
%     '18 - xCO2in           (ppm)       ';
...
%     '20 - pCO2out          (uatm)      ';
%     '21 - fCO2out          (uatm)      ';
%     '22 - HCO3out          (umol/kgSW) ';
%     '23 - CO3out           (umol/kgSW) ';
%     '24 - CO2out           (umol/kgSW) ';
%     '25 - BAlkout          (umol/kgSW) ';
%     '26 - OHout            (umol/kgSW) ';
%     '27 - PAlkout          (umol/kgSW) ';
%     '28 - SiAlkout         (umol/kgSW) ';
%     '29 - ORGAlkout        (umol/kgSW) '; %%New
%     '30 - Hfreeout         (umol/kgSW) '; %%% Changed 'Hfreein' to 'Hfreeout', svh20100827
%     '31 - RevelleFactorout ()          ';
%     '32 - OmegaCaout       ()          ';
%     '33 - OmegaArout       ()          ';
%     '34 - xCO2out          (ppm)       ';
...
% This CO2SYSOrg version is modified by Xinyu Li at
...
global fH RT;
global K0 K1 K2 KW KB KF KS KP1 KP2 KP3 KSi KI;
...
veclengths=[length(PAR1) length(PAR2) length(PAR1TYPE)...
            ...
            length(K1K2CONSTANTS) length(KSO4CONSTANTS) length(ORG) length(PKORG)];
...
[HCO3inp CO3inp BAlkinp...
    OHinp PAlkinp...
    SiAlkinp Hfreeinp ...
    HSO4inp HFinp ORGALKinp]      = CalculateAlkParts(PHic, TCc);
...
KIVEC=[K0 K1 K2 -log10(K1) -log10(K2) KW KB KF KS KP1 KP2 KP3 KSi KI];
...
[HCO3out CO3out BAlkout...
    OHout PAlkout...
    SiAlkout Hfreeout ...
    HSO4out HFout ORGALKout]      = CalculateAlkParts(PHoc, TCc);
...
KOVEC=[K0 K1 K2 -log10(K1) -log10(K2) KW KB KF KS KP1 KP2 KP3 KSi KI];
TVEC =[TB TF TS];
...
DATA=[TAc*1e6        TCc*1e6         PHic           PCic*1e6        FCic*1e6...
      ...
      PAlkinp*1e6    SiAlkinp*1e6    ORGALKinp*1e6  Hfreeinp*1e6   Revelleinp      OmegaCainp... %%% Multiplied Hfreeinp *1e6, svh20100827
      OmegaArinp     xCO2dryinp*1e6  PHoc           PCoc*1e6        FCoc*1e6...
      HCO3out*1e6    CO3out*1e6      CO2out*1e6     BAlkout*1e6     OHout*1e6...
      PAlkout*1e6    SiAlkout*1e6    ORGALKout*1e6  Hfreeout*1e6   Revelleout      OmegaCaout... %%% Multiplied Hfreeout *1e6, svh20100827
      ...
      PO4            SI       ORG    KIVEC          KOVEC           TVEC*1e6];

HEADERS={'TAlk';'TCO2';'pHin';'pCO2in';'fCO2in';'HCO3in';'CO3in';...
    'CO2in';'BAlkin';'OHin';'PAlkin';'SiAlkin';'DYEALKinp';'Hfreein';'RFin';...
    'OmegaCAin';'OmegaARin';'xCO2in';'pHout';'pCO2out';'fCO2out';...
    'HCO3out';'CO3out';'CO2out';'BAlkout';'OHout';'PAlkout';...
    'SiAlkout';'DYEALKout';'Hfreeout';'RFout';'OmegaCAout';'OmegaARout';'xCO2out';
    ...
    'pHSCALEIN';'SAL';'PO4';'SI';'ORG';'K0input';'K1input';'K2input';'pK1input';...
    'pK2input';'KWinput';'KBinput';'KFinput';'KSinput';'KP1input';'KP2input';...
    'KP3input';'KSiinput';'KIinput';'K0output';'K1output';'K2output';'pK1output';...
    'pK2output';'KWoutput';'KBoutput';'KFoutput';'KSoutput';'KP1output';...
    'KP2output';'KP3output';'KSioutput';'KIoutput';'TB';'TF';'TS';};
...
clear global K1 KP2 Pbar RT TP TempK logTempK KI TI
...
global K1 K2 KW KB KF KS KP1 KP2 KP3 KSi KI;
...
% 
% F=(WhichKs==7 | WhichKs==6 | WhichKs==8);
% if any(F)
%     KI1(F) = 0; KI2(F) = 0;
%     % Neither the GEOSECS choice nor the freshwater choice
%     % include contributions from phosphate or silicate.
...
%     d5 = 1.90997693e-2;
%     d6 = -1.56396488e-3;
%     PKI2(F)= a0 + (a1.*Sal(F).^0.5) + (a2.*Sal(F).^1.5) +(a3.*Sal(F).^2)+(a4.*Sal(F).^2.5)...
...
%    %PKI2(F) = -246.64209 + 0.315971.*Sal(F)+ 2.8855.*1e-4.*Sal(F).*Sal(F)+...
%    %(7229.23864 - 7.098137.*Sal(F) -0.057034.*Sal(F).*Sal(F))./TempK(F)+(44.493382-0.052711.* Sal(F)).*...
%    %log(TempK(F))-0.0781344.* TempK(F);      %%%Liu's equation 2011
%    
%    PKI2(F) = PKI2(F) + 0.3464; KI2(F) = 10.^(-PKI2(F)); %%%%%I temporally assume it's e2 = 2.222
...
TSiF=TSi(F); KSiF=KSi(F); TBF =TB(F);   KBF=KB(F); KIF = KI(F);
...
while any(abs(deltapH) > pHTol)
    ...
    OrgsP1    = TIF.*KIF./(KIF + H); 
    OrgsP2    = TIF.*KIF./(KIF + (10^-4.5));
    OrgAlk    = OrgsP1 -OrgsP2;
    ...
    Residual  = TAx - CAlk - BAlk - OH - PAlk - SiAlk - OrgAlk + Hfree + HSO4 + HF;
    % find Slope dTA/dpH;
...
global K0 K1 K2 KW KB KF KS KP1 KP2 KP3 KSi KI;
global TB TF TS TP TSi F TI;
K1F=K1(F);   K2F=K2(F);   KWF=KW(F); KIF = KI(F); TIF = TI(F);
KP1F=KP1(F); KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F);
...
HSO4      = TSF./(1 + KSF./Hfree); %' since KS is on the free scale
HF        = TFF./(1 + KFF./Hfree); %' since KF is on the free scale
CAlk      = TAx - BAlk - OH - PAlk - SiAlk - OrgAlk + Hfree + HSO4 + HF;
...
% ' in seawater (pH > 6) it will be equally valid on any pH scale (H terms
% ' negligible) as long as the K Constants are on that scale.
K0F=K0(F);   K1F=K1(F);   K2F=K2(F);   KWF=KW(F); KI = KI(F);
KP1F=KP1(F); KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F);
TSiF=TSi(F); KSiF=KSi(F); TBF=TB(F);   KBF=KB(F); TIF = TI(F)
TSF=TS(F);   KSF=KS(F);   TFF=TF(F);   KFF=KF(F);
vl         = sum(F); % vectorlength
pHGuess    = 8;      % this is the first guess
...
while any(abs(deltapH) > pHTol)
    ...
    HSO4      = TSF./(1 + KSF./Hfree); %' since KS is on the free scale
    HF        = TFF./(1 + KFF./Hfree);% ' since KF is on the free scale
    Residual  = TAi - CAlk - BAlk - OH - PAlk - SiAlk - OrgAlk + Hfree + HSO4 + HF;
    % '               find Slope dTA/dpH
    % '               (this is not exact, but keeps all important terms):
...
% ' negligible) as long as the K Constants are on that scale.
K1F=K1(F);   K2F=K2(F);   KWF=KW(F);
KP1F=KP1(F); KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F); KIF =KI(F);
TSiF=TSi(F); KSiF=KSi(F); TBF=TB(F);   KBF=KB(F);
TSF=TS(F);   KSF=KS(F);   TFF=TF(F);   KFF=KF(F); TIF = TI(F);
H         = 10.^(-pHi);
...
HSO4      = TSF./(1 + KSF./Hfree);% ' since KS is on the free scale
HF        = TFF./(1 + KFF./Hfree);% ' since KF is on the free scale
TActemp     = CAlk + BAlk + OH + PAlk + SiAlk + OrgAlk - Hfree - HSO4 - HF;
varargout{1}=TActemp;
end % end nested function
...
varargout{9} = HF; varargout{10} = OrgAlk;
end % end nested function
